...
var consecutive_idle_cycles: int
...
func _execute_single_action(action):
	...
	if is_hurt or is_recently_hit:
		return
		
	# Additional safety check for animation state
	if animation.is_playing() and not animation.current_animation in ["idle", "move_forward", "move_backward"]:
		# Don't interrupt non-movement animations
		return
	...
	match action:
		...
		"light_punch", "light_kick", "heavy_punch", "heavy_kick", "crouch_lightPunch", "crouch_lightKick", "crouch_heavyPunch":
			if is_on_floor() and not is_jumping and not is_attacking and not is_hurt:
		...
		"dash_forward", "dash_backward":
			if is_on_floor() and not is_jumping and not is_attacking and not is_hurt:
				var direction = 1 if action == "dash_forward" else -1
				# Adjust direction based on enemy position
				if enemy and is_instance_valid(enemy):
					if enemy.global_position.x > global_position.x:
						direction = 1 if action == "dash_forward" else -1
					else:
						direction = -1 if action == "dash_forward" else 1
		...
		"slide_forward", "slide_backward":
			if can_perform_slide():
				var direction = 1 if action == "slide_forward" else -1
				# Adjust direction based on enemy position
				if enemy and is_instance_valid(enemy):
					if enemy.global_position.x > global_position.x:
						direction = 1 if action == "slide_forward" else -1
					else:
						direction = -1 if action == "slide_forward" else 1
	...
	if Global.debug_mode:
		print(name, " - Executing action: ", action)
...
func generate_script():
	...
	if consecutive_idle_cycles >= 3:  # Reduced from 11 to be more responsive
...
func _create_new_script():
	...
	# Ensure we have a mix of rule types
	var selected_rules = []
	...
	# CRITICAL: If missing movement rules, add them forcibly
...
func _on_hurt_finished():
	# Comprehensive state reset after being hurt
	is_dashing = false
	is_jumping = false
	is_crouching = false
	is_attacking = false
	...
	# Only play idle if we're actually ready
	if is_instance_valid(self) and animation and is_on_floor():
		animation.play("idle")
...
# EMERGENCY ACTION SYSTEM - From your original pre-modified scripts
...
# SMART FALLBACK SYSTEM - From your original pre-modified scripts
