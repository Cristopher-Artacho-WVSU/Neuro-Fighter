...
func attackSystem():
	...
	if current_distance <= heavyKick_Range:
		chooseAttack("heavy_kick")
	elif current_distance <= heavyPunch_Range:
		chooseAttack("heavy_punch")
	elif current_distance <= crouch_lightKick_Range:
		chooseAttack("crouch_lightKick")
	elif current_distance <= crouch_lightPunch_Range:
		chooseAttack("crouch_lightPunch")
	elif current_distance <= crouch_heavyPunch_Range:
		chooseAttack("crouch_heavyPunch")
	elif current_distance <= lightKick_Range:
		chooseAttack("light_kick")
	elif current_distance <= lightPunch_Range:
		chooseAttack("light_punch")
...
func _on_hitbox_area_entered(area: Area2D):
	...
	if hitbox and register_hit(hitbox, area):
		print("DT Successful hit registered: ", name, " -> ", area.get_parent().name)
...
func handle_incoming_hit(attacker: Node, is_upper_body: bool, is_lower_body: bool):
	...
	var attacker_animation = "light_punch"
...
func handle_successful_hit(victim: Node, is_upper_body: bool, is_lower_body: bool):
	...
	print("DT " + name + " landed a hit on " + victim.name)
...
func debug_collision_shapes():
	print("=== DT Collision Debug for ", name, " ===")

	# HITBOXES
	for i in range(hitboxGroup.size()):
		var hitbox: Area2D = hitboxGroup[i]
		...
		for child in hitbox.get_children():
			if child is CollisionShape2D:
				print(
					"Hitbox ", i, ": ", hitbox.name,
					" | monitoring: ", hitbox.monitoring,
					" | shape: ", child.name,
	...
	for i in range(hurtboxGroup.size()):
		var hurtbox: Area2D = hurtboxGroup[i]
...
func enable_hitboxes():
	for hitbox: Area2D in hitboxGroup:
		if not is_instance_valid(hitbox):
			continue
